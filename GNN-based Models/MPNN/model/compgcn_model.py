...
class CompGCNBase(torch.nn.Module):
	...
	def forward(self, feature=None):
		...
		x = self.init_embed
		print("self.init_embed:", x.size(), r.size(), feature) # feature is None

		if feature != None:

			x = feature['entity_embedding']
			print("feature['entity_embedding']:", x.size())
		...
		print("now r is:", r.size(), self.p.compgcn_num_bases)
		x, r	= self.conv1(x, self.edge_index, self.edge_type, rel_embed=r)
		print("after r is:", r.size())
